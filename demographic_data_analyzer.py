...
def calculate_demographic_data(print_data=True):
    ...
    higher_education_rich = round(len(df[(df['education'].isin(['Bachelors','Masters', 'Doctorate'])) & (df['salary']=='>50K')])*100/len(df[df['education'].isin(['Bachelors','Masters', 'Doctorate'])]),1)
    ...
    num_min_workers = round(len(df[(df['hours-per-week']==min_work_hours) & (df['salary']=='>50K')])*100/len(df[df['hours-per-week']==min_work_hours]),1)
